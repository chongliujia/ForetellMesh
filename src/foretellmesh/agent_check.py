...
def check_agent_workflow(config_path: Path, fixture_path: Path, output: Path, workflow="reviewed_forecast", *, engine="serial") -> dict:
    if output.exists():
    ...
    if engine == "langgraph":
        ...
        raise ValidationError("unknown orchestration engine")
    result = runner(config, backend).run(record.forecast_input, workflow=workflow, mode="capability")
    ...
    if engine == "langgraph":
        ...
        report["orchestration"] = {"engine": engine, "langgraph_version": version("langgraph")}
