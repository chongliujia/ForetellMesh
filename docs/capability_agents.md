...
能力配置仍标记为 `planned`，它表示架构计划，不是 checkpoint 晋级注册表。`research_tool_lora` 已完成两版候选训练与评估：首版的流程回归已通过新提示修复，但第二版在新任务约束下退步，两版均未晋级默认；`forecast_lora` 尚未训练，详见 [首次训练](research_tool_lora.md) 与 [第二版三组对照](research_tool_lora_v2.md)。角色通过不同提示和输出契约区分，不为每个 Agent 启动一个完整 8B 模型。Orchestrator 使用确定性的任务路由，不额外调用一个模型。
