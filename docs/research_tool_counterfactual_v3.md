...
本页记录**数据生成、确定性回放、防泄漏校验和分词预检**阶段，当时尚未训练第三版 LoRA，也没有模型性能结论。该数据准备工作已提交为 `05ad8bb`，此前代码与实验文档提交为 `9fc4a8f`。后续已完成平衡选样、第三版训练及 588 项生成对照，结果见 [第三版训练与评估](research_tool_lora_v3.md)。
...
这里的训练配置仅用于检查现有编码接口和长度预算，该命令不会执行训练。完整数据池中新增未知项任务占比很高，后续训练必须先固定按任务与事件组平衡的预算，避免工具任务被稀释；再从新验证情景冻结生成对照，连同原有工具、证据、完整流程和旧诊断一起评估。后续第三版实验已按此流程执行，见上方报告。原 24 条角色验证不覆盖新增能力，训练损失不足以完成验收。正反证据判断仍是独立待改善项。
