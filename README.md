...
真实数据路线以**已结算预测市场的历史回放**为主：已归档 **Polymarket 10 个合约 + Kalshi 22 个合约**，在两次 FOMC 会议前 7 天、前 1 天构建 **64 条观察**；结果与官方声明交叉核验，事前证据、历史报价与结果账本分离。规则版本、部分结算证明及教师答案仍待补齐，尚无可直接微调的真实 SFT 样本，见 [历史数据与质量报告说明](docs/historical_prediction_markets.md)。
...
已加入**能力路由、串行 Agent 编排、PEFT 请求级切换和 Brier 奖励**。首版规划共享 `research_tool_lora` 与 `forecast_lora` 两类能力；研究/工具能力已训练并评估两版候选，预测能力尚未训练，均未晋级默认模型。使用方式、实际验证边界和系统消融计划见 [多 Agent / LoRA / RL 说明](docs/capability_agents.md)。
...
已补充**缺失参数与任务范围的反事实候选数据**：训练 2,336 条、验证 840 条、测试 840 条，同一数值情景的参数删减、范围、字段别名、中英文与角色变体按组隔离。确定性回放和本地 tokenizer 预检通过，训练最长 957 token；本轮完成数据准备，尚未训练第三版 LoRA。数据设计、检查结果和后续平衡训练要求见 [第三版候选数据说明](docs/research_tool_counterfactual_v3.md)。
...
1. 完善已接入的 Polymarket / Kalshi 历史回放：补规则版本和结算证明，扩充事前证据与独立事件，审核关联事件及基准重叠。前瞻快照继续保留；既有 6 条保留集样本继续用于管线检查。
...
目前已有数据管线、确定性基线评估、串行多 Agent 骨架、PEFT 推理后端、LoRA 资源测试及合成能力训练/对照评估入口。完整在线检索、真实事件模型评测、GRPO 训练器和 vLLM 服务尚未接通；已有真实样本上的确定性基线分数，尚无真实事件上的模型预测性能结论。项目约束见 [AGENTS.md](AGENTS.md)。
