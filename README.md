...
历史数据新增**用途分开的准入预检**与 10 个 Polymarket 合约的链上状态补证。缺教师答案只阻塞 SFT；规则版本、精确结算和基准重叠等检查仍严格保留，64 条历史候选的正式评分准入仍为 0。三条 Agent 流程的真实对照策略已版本化，实际评测须等待合格数据发布，见 [历史准入与补证说明](docs/historical_replay_admission.md)。
